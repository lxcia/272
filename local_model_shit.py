...
class LocalModel(nn.Module):
    def __init__(self, input_size=13, layer_sizes=[16, 32, 16], output_size=10):
    ...
    def train(self, train_data, train_labels, epochs=10000, learn=0.000001):
        ...
        for epoch in range(epochs):
            ...
            if epoch % 100 == 0:
                ...
                accuracy = np.average(np.equal(np.array(predictions), np.array(train_labels)))
    ...
    def test(self, test_data, test_labels):
        # Predict on test set
        test_data = test_data.to(torch.float32).requires_grad_()

        outputs = self.forward(test_data)
        max_probs, predictions = torch.max(outputs, 1)

        ground_truth_labels = test_labels.long()
